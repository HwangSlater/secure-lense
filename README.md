...
- **파일 해시 계산**: MD5, SHA1, SHA256 해시 생성 (위협 인텔리전스 연동용)
- **파일 크기 분석**: 비정상적으로 작거나 큰 파일 탐지
- **파일명 패턴 분석**: 이중 확장자, 의심 키워드, 랜덤 파일명 탐지
- **Base64 인코딩 탐지**: 파일 내 Base64 인코딩된 데이터 탐지 (난독화 탐지)
